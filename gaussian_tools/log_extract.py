...
This script extracts the last structure from Gaussian log files a creates gjf
files with the coordinates and the same keywords used in it.
...
for file in files:
    ...
    # Parameters for number of processors and memory for the calculation
    # Uses 40 processors and 200GB of memory by default
    try:
        nproc = text[first_find(text,"nproc")]
    except IndexError:
        try:
            nproc = text[first_find(text,"nprocshared")]
        except:
            nproc = '40'
    ...
    # Extracts the keywords used in the previous calculation. 
    # Not sure if it works if they occupy more than 1 line
    ...
    # Last coordinates in the log file:
    coord = text
